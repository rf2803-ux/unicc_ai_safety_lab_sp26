...
from tests.helpers import sample_case_file, sample_final_output, sample_judge_output, sample_system_case
...
def test_pipeline_creates_expected_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result.system_case_path.exists()
...
def test_pipeline_accepts_system_case_directly(tmp_path: Path, monkeypatch) -> None:
    ...
    result = pipeline_module.evaluate_system_case(sample_system_case(), config)
